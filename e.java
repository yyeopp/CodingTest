...
public class SWEA1873 {
	...
	static void move(int[] carPos, char shape, char dir, char[][] map) {
		if (dir == 'U') {
			shape = car[0];
	...
	static void shoot(int[] carPos, char shape, char dir, char[][] map) {
			...
			for (int tc = 1; tc <= T; tc++) {
				...
				for (int i = 0; i < N; i++) {
					...
					if (order[i] == dir[0] || order[i] == dir[1] || order[i] == dir[2] || order[i] == dir[3]) {
						move(carPosition, shape, order[i], map);
					}
					if (order[i] == 'S') {
						shoot(carPosition, shape, order[i], map);
					}
